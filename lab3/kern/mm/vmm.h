...
struct vma_struct {
    struct mm_struct *vm_mm; // the set of vma using the same PDT 所属的内存管理器
    uintptr_t vm_start;      // start addr of vma      
    uintptr_t vm_end;        // end addr of vma, not include the vm_end itself
    uint32_t vm_flags;       // flags of vma   标志位包括可读、可写、可执行
...
// the control struct for a set of vma using the same PDT 虚拟内存管理器
struct mm_struct {
    list_entry_t mmap_list;        // linear list link which sorted by start addr of vma  指向线性区对象的链表头
    struct vma_struct *mmap_cache; // current accessed vma, used for speed purpose   当前的虚拟内存区
    pde_t *pgdir;                  // the PDT of these vma   页表
    int map_count;                 // the count of these vma   虚拟内存区数
